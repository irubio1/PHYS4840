...
l = .40   # Length of pendulum arms (m)
...
def equations(r):
    ...
    denom1 = 3 -np.cos(2*theta1-2*theta2)
    denom2 = 3 -np.cos(2*theta1-2*theta2)

    fomega1 = -omega1**2*np.sin(2*theta1-2*theta2)+2*omega2*np.sin(theta1-theta2)+(g/l)\
    *(np.sin(theta1-2*theta2)-np.sin(theta2))/denom1


    fomega2 = 4*omega1**2*np.sin(theta1-theta2)+omega2**2*np.sin(2*theta1-2*theta2)+2*(g/l)\
    *(np.sin(2*theta1-theta2)-np.sin(theta2))/denom2
...
def rk4_step(r, dt):
    k1 = dt*equations(r)
    k2 = dt*equations(r+.5*k1)
    k3 = dt*equations(r+.5*k2)
    k4 = dt*equations(r+.5*k3)
